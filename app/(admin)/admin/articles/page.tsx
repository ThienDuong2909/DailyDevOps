...
function getStatusBadge(status: string) {
    switch (status) {
        ...
        case 'REVIEW':
            return { bg: 'bg-violet-500/10', text: 'text-violet-300', border: 'border-violet-500/20', dot: 'bg-violet-300', label: 'In Review' };
        case 'ARCHIVED':
...
export default function ArticlesPage() {
    ...
    const handleNotionImport = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const response = await new Promise<any>((resolve, reject) => {
                ...
                xhr.upload.onprogress = (progressEvent) => {
                    ...
                    const percent = Math.min(
                        72,
                        Math.max(8, Math.round((progressEvent.loaded / progressEvent.total) * 72))
                    );
                ...
                xhr.onload = () => {
                    try {
                        ...
                        reject(new Error(payload?.error || 'Khong the import bai viet Notion'));
                    } catch {
            ...
            toast.error(error instanceof Error ? error.message : 'Khong the import bai viet tu Notion');
    ...
    return (
        <div className="mx-auto flex max-w-[1400px] flex-col gap-6">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-[color:var(--text-main-theme)]">Quan ly bai viet</h1>
            ...
            {isImporting ? (
                <div className="theme-panel rounded-2xl p-4">
                    <div className="flex items-center justify-between gap-4">
                        <div>
                            <p className="text-sm font-bold text-[color:var(--text-main-theme)]">Importing Notion export</p>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full border-collapse text-left">
                        ...
                        <tbody className="divide-y theme-border text-sm text-[color:var(--text-main-theme)]">
                            {loading ? (
                                <tr>
                                    <td colSpan={5} className="theme-muted px-6 py-16 text-center">
                                ...
                                posts.map((post) => {
                                    ...
                                    return (
                                        <tr key={post.id} className="group transition-colors hover:bg-[color:var(--surface-muted)]">
                                            <td className="px-6 py-4">
                                                <div className="flex flex-col gap-1">
                                                    ...
                                                    <span className="theme-muted text-xs font-mono">/{post.slug}</span>
                                                    {post.excerpt ? (
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex items-center justify-end gap-2 opacity-100 transition-opacity sm:opacity-60 group-hover:opacity-100">
                                                    ...
                                                    {post.status === 'REVIEW' ? (
                                                        <>
                                                            <button
                                                                onClick={() => void apiClient.post(`/api/v1/posts/${post.id}/approve`).then(() => {
                                                                    toast.success('Da duyet va publish bai viet');
                                                                    void fetchPosts(true);
                                                                }).catch(() => {
                                                            ...
                                                            <button
                                                                onClick={() => {
                                                                    ...
                                                                    void apiClient.post(`/api/v1/posts/${post.id}/reject`, { rejectionReason }).then(() => {
                                                        ...
                                                        <button
                                                            onClick={() => void apiClient.post(`/api/v1/posts/${post.id}/submit-review`).then(() => {
                ...
                {totalPages > 1 ? (
                    <div className="theme-border flex items-center justify-between border-t px-6 py-4">
                        <div className="theme-muted text-sm">
                            Page <span className="font-medium text-[color:var(--text-main-theme)]">{currentPage}</span> of <span className="font-medium text-[color:var(--text-main-theme)]">{totalPages}</span> ({totalPosts} total)
                        </div>
