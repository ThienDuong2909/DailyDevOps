...
export default function AdminAccountPage() {
        ...
        try {
            ...
            if (pendingAvatarFile) {
                ...
                if (!uploadResponse.ok || !uploadPayload?.data?.url) {
                    throw new Error(uploadPayload?.error || uploadPayload?.message || 'Khong the upload avatar');
                }
    ...
    const handleSetupMfa = async () => {
        try {
            ...
            const response = await apiClient.post<{ data?: MfaSetupState }>('/api/v1/auth/mfa/setup');
        ...
        <div className="mx-auto flex w-full max-w-[1280px] flex-col gap-6">
            <section className="theme-panel overflow-hidden rounded-[28px]">
                <div className="flex flex-col gap-4 p-6 lg:flex-row lg:items-center lg:justify-between lg:p-8">
                    ...
                    <div className="rounded-2xl border border-primary/20 bg-primary/5 px-5 py-4">
                        <p className="text-xs uppercase tracking-[0.2em] text-primary">Current admin</p>
            ...
            <section className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
                ...
                >
                    ...
                    <div className="mt-5">
                        ...
                        <p className="theme-muted text-xs leading-6">
                            URL anh khong hien thi tren giao dien admin. Anh moi chi duoc upload len
                            storage khi ban bam <span className="font-semibold text-[color:var(--text-main-theme)]">Save Profile</span>.
                        </p>
                    </div>
                ...
                <div className="space-y-6">
                    <div className="theme-panel rounded-3xl p-6">
                        ...
                        <div className="mt-5 space-y-3">
                            <div className="rounded-2xl border border-[color:var(--border-theme)] px-4 py-3">
                                <p className="theme-muted text-xs uppercase tracking-wide">Role</p>
                                <p className="mt-1 text-sm font-semibold text-[color:var(--text-main-theme)]">
                                    {user.role}
                                </p>
                            </div>
                            <div className="rounded-2xl border border-[color:var(--border-theme)] px-4 py-3">
                                <p className="theme-muted text-xs uppercase tracking-wide">Email verified</p>
                                <p className="mt-1 text-sm font-semibold text-[color:var(--text-main-theme)]">
                                    {user.emailVerifiedAt ? 'Verified' : 'Pending'}
                                </p>
                            </div>
                            <div className="rounded-2xl border border-[color:var(--border-theme)] px-4 py-3">
                                <p className="theme-muted text-xs uppercase tracking-wide">Joined</p>
                                <p className="mt-1 text-sm font-semibold text-[color:var(--text-main-theme)]">
                                    {formatDate(user.createdAt)}
                                </p>
                            </div>
                            <div className="rounded-2xl border border-[color:var(--border-theme)] px-4 py-3">
                                <p className="theme-muted text-xs uppercase tracking-wide">MFA status</p>
                                <p className="mt-1 text-sm font-semibold text-[color:var(--text-main-theme)]">
                    ...
                    <form onSubmit={handlePasswordChange} className="theme-panel rounded-3xl p-6">
                        <h2 className="text-xl font-bold text-[color:var(--text-main-theme)]">
                            Change Password
                        </h2>
                ...
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                        ...
                        <p className="theme-muted mt-2 max-w-3xl text-sm leading-7">
                            Bao ve tai khoan quan tri bang ma TOTP 6 so tu Google Authenticator,
                ...
                {!user.mfaEnabled ? (
                    <div className="mt-6 space-y-5">
                        {!mfaSetup ? (
                            ...
                            <form onSubmit={handleEnableMfa} className="grid gap-6 xl:grid-cols-[0.8fr_1.2fr]">
                                ...
                                <div className="space-y-4">
                                    <div className="rounded-2xl border border-primary/20 bg-primary/5 p-4 text-sm text-[color:var(--text-main-theme)]">
                                        Sau khi scan xong, nhap lai mat khau hien tai va ma 6 so dang
                                        hien tren authenticator de kich hoat MFA.
                                    </div>
                                    <input
                                        type="password"
                    ...
                    <form onSubmit={handleDisableMfa} className="mt-6 grid gap-4 xl:grid-cols-[1fr_1fr_auto] xl:items-end">
                        <input
                            type="password"
