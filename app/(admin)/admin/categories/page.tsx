...
export default function CategoriesAdminPage() {
  ...
  const fetchCategories = async () => {
    const payload =
      await apiClient.get<CategoriesPayload>("/api/v1/categories");
  ...
  const handleSubmit = async (event: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.error(
        editingId ? "Khong the cap nhat category" : "Khong the tao category",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const handleDelete = async (category: Category) => {
    if (!globalThis.window.confirm(`Xoa category "${category.name}"?`)) {
      return;
    }
