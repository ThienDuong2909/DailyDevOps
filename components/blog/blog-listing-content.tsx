...
type ApiListPayload<T> = { data?: T[] } | T[];
...
function parseNestedPostList(
  nested: unknown,
  meta?: { total?: number; totalPages?: number },
) {
  if (Array.isArray(nested)) {
    return {
      posts: nested as Post[],
...
function extractPosts(payload: unknown): {
  ...
  return parseNestedPostList(response.data, response.meta) ?? emptyPostList();
}

function resolveListData<T>(payload: ApiListPayload<T>): T[] {
  if (Array.isArray(payload)) {
    return payload;
  }
...
function getSkeletonCardCount() {
  return Math.min(POSTS_PER_PAGE, 9);
}
...
export function BlogListingContent() {
  ...
  const sortOptions = useMemo<Record<SortOption, SortConfig>>(
    () => ({
      latest: {
        label: dictionary.blog.sortLatest,
        sortBy: "publishedAt",
        sortOrder: "desc",
      },
      popular: {
        label: dictionary.blog.sortPopular,
        sortBy: "viewCount",
        sortOrder: "desc",
      },
      oldest: {
        label: dictionary.blog.sortOldest,
        sortBy: "publishedAt",
        sortOrder: "asc",
      },
    }),
    [
      dictionary.blog.sortLatest,
      dictionary.blog.sortOldest,
      dictionary.blog.sortPopular,
    ],
  );
  ...
  useEffect(() => {
    ...
    const fetchPosts = async () => {
      ...
      try {
        const sortConfig = sortOptions[currentSort] || sortOptions.latest;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const cats = resolveListData(res);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const result = resolveListData(res);
        if (alive)
          setPopularPosts(result.filter((p) => (p.viewCount ?? 0) > 0));
      } catch {
        /* silent */
      } finally {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const tags = resolveListData(res);
        if (alive) {
          setTagCloud(
            tags
      ...
      <header className="space-y-6">
        <div className="flex flex-col gap-1">
          <h1 className="text-3xl font-black tracking-tight text-[var(--text-main-theme)] md:text-4xl">
            {dictionary.blog.allPostsTitle}
          </h1>
          <p className="text-sm text-[var(--text-muted-theme)] md:text-base">
            {dictionary.blog.allPostsDescription.replace(
              "{count}",
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative max-w-md flex-1">
            ...
            <input
              type="text"
              placeholder={dictionary.blog.searchArticlesPlaceholder}
              value={searchInput}
            ...
            {searchInput && (
              <button
                ...
                type="button"
                aria-label={dictionary.blog.clearSearch}
              >
                <X className="size-4" />
              </button>
            )}
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-1 rounded-xl bg-[var(--surface-muted)] p-1">
              {(Object.entries(sortOptions) as [SortOption, SortConfig][]).map(
                ([key, config]) => (
                  <button
                    key={key}
        ...
        <div className="min-w-0 flex-1">
          {isLoading ? (
            <div className="grid grid-cols-1 gap-7 md:grid-cols-2 xl:grid-cols-3">
              {Array.from({ length: getSkeletonCardCount() }, (_, i) => (
                <div
                  key={i}
            ...
            <div className="rounded-2xl border border-dashed border-[var(--border-soft-theme)] bg-[var(--surface-elevated)] px-6 py-16 text-center">
              ...
              <h2 className="text-lg font-bold text-[var(--text-main-theme)]">
                {currentSearch
                  ? dictionary.blog.noSearchResults
                  : dictionary.blog.noPostsYet}
              </h2>
              <p className="mt-2 text-sm text-[var(--text-muted-theme)]">
                {currentSearch
                  ? dictionary.blog.noSearchResultsBody.replace(
                      "{query}",
                      currentSearch,
                    )
                  : dictionary.blog.noPostsYetBody}
              </p>
          ...
          {!isLoading && totalPages > 1 && (
            <div className="mt-8 flex flex-col gap-4 rounded-2xl border border-[var(--border-soft-theme)] bg-[var(--surface-elevated)] px-5 py-4 md:flex-row md:items-center md:justify-between">
              <p className="text-sm text-[var(--text-muted-theme)]">
                {dictionary.blog.pageProgress
                  .replace("{page}", String(currentPage))
                  .replace("{totalPages}", String(totalPages))}
                <span className="ml-2 text-[var(--text-soft-theme)]">
                  ·{" "}
                  {dictionary.blog.postCount.replace(
                    "{count}",
                    String(totalPosts),
                  )}
                </span>
              ...
              <div className="flex flex-wrap gap-2">
                <Link
                  href={
                    currentPage > 1
                      ? withLocale(`/blog?page=${currentPage - 1}`, locale)
                    ...
                    <span key={pn} className="contents">
                      ...
                      <Link
                        href={
                          pn === 1
                            ? withLocale("/blog", locale)
                            : withLocale(`/blog?page=${pn}`, locale)
                        }
                        className={cn(
                ...
                <Link
                  href={withLocale(
                    `/blog?page=${Math.min(totalPages, currentPage + 1)}`,
                    locale,
                  )}
              ...
              <div className="overflow-hidden rounded-2xl border border-[var(--border-soft-theme)] bg-[var(--surface-elevated)]">
                <div className="px-5 pb-1 pt-5">
                  <h3 className="text-base font-extrabold tracking-tight text-[var(--text-main-theme)]">
                    {dictionary.blog.popularMostRead}
                  </h3>
                </div>
          ...
          <div className="relative flex flex-col gap-6 p-8 md:flex-row md:items-center md:justify-between md:p-12">
            <div className="max-w-2xl space-y-2 text-center md:text-left">
              <h2 className="text-2xl font-black tracking-tight text-white md:text-3xl">
                {dictionary.blog.newsletterCtaTitle}
              </h2>
              <p className="text-sm leading-6 text-cyan-50 md:text-base">
                {dictionary.blog.newsletterCtaBody}
              </p>
            </div>
            <div className="w-full max-w-md">
              <NewsletterSignupForm
                buttonClassName="bg-surface-dark hover:bg-gray-900"
                buttonLabel={dictionary.blog.newsletterCtaButton}
                helperText={dictionary.blog.newsletterCtaHelper}
