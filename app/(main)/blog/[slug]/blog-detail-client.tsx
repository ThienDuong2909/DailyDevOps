...
function PostComments({ post, isAuthenticated, user, form, setForm, submitting, handleCommentSubmit, handleShare }: any) {
    return (
        <section className="mt-14">
            ...
            <div className="theme-surface mb-8 rounded-2xl p-6">
                {!isAuthenticated ? (
                    <div className="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                        <input className="theme-input w-full rounded-2xl px-4 py-3 text-sm" placeholder="Your name" value={form.authorName} onChange={(e) => setForm((p: any) => ({ ...p, authorName: e.target.value }))} />
                        <input className="theme-input w-full rounded-2xl px-4 py-3 text-sm" placeholder="Your email" type="email" value={form.authorEmail} onChange={(e) => setForm((p: any) => ({ ...p, authorEmail: e.target.value }))} />
                    </div>
                ) : (
            ...
            <div className="space-y-5">
                {post.comments?.length ? post.comments.map((comment: any) => (
                    <div
                        key={comment.id}
            ...
            <section className="mx-auto max-w-[1280px] overflow-x-clip px-4 pt-6 sm:pt-8 lg:px-8 lg:pt-10">
                ...
                <div className="grid grid-cols-1 items-start gap-8 lg:grid-cols-[minmax(0,7fr)_minmax(280px,3fr)]">
                    <div className="min-w-0">
                        ...
                        <NewsletterSection />
                        <PostComments post={post} isAuthenticated={isAuthenticated} user={user} form={form} setForm={setForm} submitting={submitting} handleCommentSubmit={handleCommentSubmit} handleShare={handleShare} />
                    </div>

                    <PostSidebar derivedTocItems={derivedTocItems} tocNavRef={tocNavRef} scrollToHeading={scrollToHeading} effectiveActiveTocId={effectiveActiveTocId} post={post} handleShare={handleShare} relatedPosts={relatedPosts} popularPosts={popularPosts} />
                </div>
            </section>
