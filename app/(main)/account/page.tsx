...
const NON_USERNAME_CHARACTERS_REGEX = /[^a-z0-9\s-]/g;
const USERNAME_WHITESPACE_REGEX = /\s+/g;
const USERNAME_REPEATED_HYPHENS_REGEX = /-+/g;
const NON_EMAIL_USERNAME_CHARACTERS_REGEX = /[^a-z0-9-]/g;

function buildPublicUsername(
  firstName?: string,
  lastName?: string,
  email?: string,
) {
  let fullName = `${firstName || ""} ${lastName || ""}`
    .trim()
    .toLowerCase()
    .replaceAll(NON_USERNAME_CHARACTERS_REGEX, "")
    .replaceAll(USERNAME_WHITESPACE_REGEX, "-")
    .replaceAll(USERNAME_REPEATED_HYPHENS_REGEX, "-");
  ...
  let emailUsername = String(email || "")
    .split("@")[0]
    .toLowerCase()
    .replaceAll(NON_EMAIL_USERNAME_CHARACTERS_REGEX, "-")
    .replaceAll(USERNAME_REPEATED_HYPHENS_REGEX, "-");

  while (emailUsername.startsWith("-")) {
    emailUsername = emailUsername.slice(1);
  }

  while (emailUsername.endsWith("-")) {
    emailUsername = emailUsername.slice(0, -1);
  }
...
export default function AccountPage() {
  ...
  const handleAvatarUpload = async (
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Khong the upload avatar",
      );
    } finally {
    ...
    <div className="flex w-full max-w-[1120px] flex-col gap-8">
      <section className="rounded-[32px] bg-slate-950 px-6 py-10 text-white shadow-xl md:px-10 md:py-14">
        ...
        <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-300 md:text-base">
          Update your public author profile, rotate your password, and access
          privacy controls from one place.
        </p>
      </section>
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-7 text-text-sub dark:text-gray-400">
              Theo doi cac bai viet gan day cua ban qua cac trang thai draft,
              review, schedule va publish.
            </p>
          </div>
      ...
      <section className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-surface-dark">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-7 text-text-sub dark:text-gray-400">
              Theo doi comment moi tren bai viet cua ban va xem trang thai
              moderation hien tai.
            </p>
          </div>
          <Link
          ...
          <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-surface-dark">
            ...
            {["ADMIN", "MODERATOR", "EDITOR"].includes(user.role) ? (
              <div className="mt-4 rounded-2xl border border-primary/20 bg-primary/5 p-4 text-sm text-text-main dark:text-white">
                MFA duoc khuyen nghi rat cao cho tai khoan co quyen quan tri va
                bien tap.
              </div>
            ) : null}
