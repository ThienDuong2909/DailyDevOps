...
function formatPublishedLabel(post: Post, locale: string) {
  const candidate = post.publishedAt || post.createdAt || post.updatedAt;

  if (!candidate) {
    return locale === "en" ? "Recently" : "Gần đây";
  }
...
export function PostCard({ post, imageClassName }: PostCardProps) {
  ...
  const publishedLabel = formatPublishedLabel(post, locale);
  const imageUrl =
  ...
  const initials =
    `${post.author?.firstName?.[0] || "D"}${post.author?.lastName?.[0] || "D"}`.toUpperCase();
  ...
  return (
    <article className="group relative flex h-full flex-col overflow-hidden rounded-2xl bg-[var(--surface-elevated)] shadow-[0_2px_12px_rgba(15,23,42,0.06)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_12px_32px_rgba(15,23,42,0.12)]">
      <div
        className={`relative aspect-[16/9] w-full overflow-hidden ${imageClassName || ""}`}
      >
        <Link
        ...
        <Link
          href={
            post.category?.slug
              ? withLocale(`/category/${post.category.slug}`, locale)
      ...
      <div className="flex flex-1 flex-col gap-3 px-5 pb-5 pt-4">
        ...
        <div className="mt-auto flex items-center gap-2.5 pt-1">
          ...
          <div className="flex items-center gap-1.5 text-[13px] text-[var(--text-muted-theme)]">
            <span className="font-semibold text-[var(--text-main-theme)]">
              {authorName}
            </span>
